...
const base =
  "https://api.thegraph.com/subgraphs/name/danielsmith0630/fxdx-base-actions";
...
const basestats = new ApolloClient({
  uri: base,
  cache: new InMemoryCache(),
});
...
export const baseUserTradeStats = async () => {
  try {
    const data = await basestats.query({
      query: userTradesStatsQuery,
    });
...
export const basewinloss = async (aggregatedData) => {
  try {
    const data = aggregatedData.map(async (user) => {
      const wl = await basestats.query({
        query: userTradeDetailsQuery,
        variables: { account: user.account },
      });
